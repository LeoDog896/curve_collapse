...
function equation(x){
    //Your Equation Here - use X for the input value. Will return Y when called.
    return Math.pow(x,2);
  }
  ...
  function deriv(x,degree,rB_input,rB_output){
    ...
    var delta=Math.pow(10,(-1*degree));
    ...
    var result=(equation(x+delta)-equation(x-delta))/(2*delta);
  ...
  function integ(a,b,degree,rB_input,rB_output){
    ...
    for (var i=0; i<delta; i++){
      ...
      var bound1=a+range*(i/delta);
      var bound2=a+range*((i+1)/delta);
  ...
  //
  function trueNum(boolean){if (boolean&&(boolean!=undefined)) return 1; else return 0;}
  ...
  //
  function curveLength(a,b,degree,rB_input,rB_internal,rB_output,sD_enable,sD_inputDeg,sD_integDeg,sD_derivDeg){
    ...
    if (sD_inputDeg==undefined||sD_integDeg==undefined||sD_derivDeg==undefined) {sD_inputDeg=1;sD_integDeg=1;sD_derivDeg=1;}
    
    //Makes sure degree is a whole number
    //If sD_enable is true, overrides with sD_integDeg
    degree=trueNum(!sD_enable)*Math.round(degree)+trueNum(sD_enable)*Math.round(sD_integDeg);
    //If sD_enable is true, uses sD_derivDeg for curveLengthAt()
    //Else, uses degree
    var derivDeg=trueNum(!sD_enable)*degree+trueNum(sD_enable)*Math.round(sD_derivDeg);
    //Sets the number of subdivisions to be taken as delta
    var delta=Math.pow(10,degree);
    //Alters a and b if rB_input is true
    //If sD_enable is also true, uses sD_inputDeg
    //Else uses delta
    var roundDec=trueNum(!sD_enable)*delta+trueNum(sD_enable)*Math.pow(10,Math.round(sD_inputDeg));
    ...
    for (var i=0; i<delta; i++){
      ...
      result+=( ( (curveLengthAt(bound1,derivDeg,rB_internal) + curveLengthAt(bound2,derivDeg,rB_internal))/2 )*(range/delta) );
    }
    //Rounds result if rB_output is true
    result+=trueNum(rB_output)*(Math.round(result*delta)/delta-result);
    //Returns the final value
  ...
  //                          Intensive calculation.
  //
  function pointExtrapolate(a,b,n,degree,rB_input,rB_internal,rB_integRes,rB_randomCh,rB_xyValue,sD_enable,sD_inputDeg,sD_integDeg,sD_derivDeg,sD_roundDeg){
    ...
    if (sD_inputDeg==undefined||sD_integDeg==undefined||sD_derivDeg==undefined||sD_roundDeg==undefined) {sD_inputDeg=1;sD_integDeg=1;sD_derivDeg=1;sD_roundDeg=1;}
    ...
    //Input degree
    sD_inputDeg=trueNum(!sD_enable)*Math.round(degree)+trueNum(sD_enable)*Math.round(sD_inputDeg);
    //Integral degree
    sD_integDeg=trueNum(!sD_enable)*Math.round(degree)+trueNum(sD_enable)*Math.round(sD_integDeg);
    //Derivative degree
    sD_derivDeg=trueNum(!sD_enable)*Math.round(degree)+trueNum(sD_enable)*Math.round(sD_derivDeg);
    //Rounding degree
    sD_roundDeg=trueNum(!sD_enable)*Math.round(degree)+trueNum(sD_enable)*Math.round(sD_roundDeg);
    ...
    var roundDec=Math.pow(10,sD_inputDeg);
    a+=trueNum(rB_input)*(Math.round(a*roundDec)/roundDec-a);
    b+=trueNum(rB_input)*(Math.round(b*roundDec)/roundDec-b);
    
    //STEP 1 - Find total and partial curve lengths
    //get length of total function curve from x=a to x=b
    var lengthTotal=curveLength(a,b,sD_integDeg,false,rB_internal,rB_integRes,sD_enable,sD_inputDeg,sD_derivDeg,sD_integDeg);
    //divide into n-1 sections to be placed making n points
    //as n points can make n-1 line segments
    var lengthSection=lengthTotal/(n-1);
    ...
    for (var i=0; i<n-2; i++){
      ...
      var randomAssign=Math.random()*(b-listX[i]);
      //Rounds randomAssign if rB_randomCh is true using sD_inputDeg or degree
      randomAssign+=trueNum(rB_randomCh)*(Math.round(randomAssign*sD_inputDeg)/sD_inputDeg-randomAssign);
      var randomChoose=listX[i]+randomAssign;
      //Gets the result of the curve length from listX[i], or x(i+1) to randomChoose
      var randomChooseResult=curveLength(listX[i],randomChoose,sD_integDeg,false,rB_internal,rB_integRes,sD_enable,sD_inputDeg,sD_derivDeg,sD_integDeg);
      ...
      var testScore=2 * ( ( trueNum(randomChooseResult>lengthSection) ) - 0.5 );
      //comparison to check the test score with itself, used in later code
      var testScoreCheck=testScore;
      ...
      while (testScoreCheck==testScore){
        ...
        randomChooseResult=curveLength(listX[i],randomChoose,sD_integDeg,false,rB_internal,rB_integRes,sD_enable,sD_inputDeg,sD_derivDeg,sD_integDeg);
      ...
      var closeRep=Math.round(Math.pow(3.321918095,trueNum(!sD_enable))*sD_roundDeg);
    ...
    for (var v=0;v<listX.length;v++){
      listX[v]+=trueNum(rB_input)*(Math.round(listX[v]*roundDec)/roundDec-listX[v]);
      listY[v]+=trueNum(rB_input)*(Math.round(listY[v]*roundDec)/roundDec-listY[v]);
    }
